...
function filterValues(respObject) {
  log("Template storage: ", templateStorage);
  if (getTaggingErrors(respObject)) {
    log(
      "tagging errors:",
      getTaggingErrors(respObject),
      "and respObject: ",
      respObject
    );
  }
  ...
  if (currentUrlHasUtm(getUrl()) && !respObject.campaign) {
    respObject.medium = "social_cpc";
    respObject.source = undefined;
    return respObject;
  }
  if (varSource === "t.co") {
